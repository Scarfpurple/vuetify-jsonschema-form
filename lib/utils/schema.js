...
schemaUtils.prepareFullSchema = (schema, value, options) => {
    ...
    fullSchema.allOf.forEach(obj => {
      if (obj.if) {
        if (!options.validator) {
          ...
          console.log(obj)
          const validate = options.validator(obj.if)
          const error = validate(value)
          console.log(error)
          if (!error && obj.then) {
            extendSchema(fullSchema, obj.then)
          }
          if (error && fullSchema.else) {
            extendSchema(fullSchema, obj.else)
          }
        }
      }
    })
  ...
  const _fullSchema = copy(fullSchema)
  _fullSchema.properties = {}
  fullSchema.properties.forEach(prop => {
    _fullSchema.properties[prop.key] = prop
  })
  console.log(_fullSchema)
  return _fullSchema
}
